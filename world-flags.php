...
function world_flags_extra_page() {
	...
	if ( isset( $_POST['get_database'] ) ) {
		if ( world_flags_cron() ) {
			...
			if ( $rows > 0 ) {
				$message = __( "IP list successfully downloaded and $rows were inserted into database.", 'world_flags' );
			} else {
	...
	echo '						' . __( 'Do not use this button often. Once a week or even once a month should work if you have disabled the scheduled download. Please be aware that software77.net may ban your IP if you send many download requrests. Refer to the policy on their website for more information.', 'world_flags' ) . '</td>';
	echo '				</tr>';
	echo '		</table>';
	echo '	</fieldset>';
	echo '</form>';
...
function world_flags_db_notice( $plugin ) {
	$db_exists = file_exists( ABSPATH . FLAGS_WEBDIR . "/data/IpToCountry.csv.gz" ) && file_exists( ABSPATH . FLAGS_WEBDIR . "/data/IpToCountry.csv" ) ;
 	if( $plugin == 'world-flags/world-flags.php' && !$db_exists && function_exists( "admin_url" ) )
		echo '<td colspan="5" class="plugin-update">' . sprintf( __( 'You must download the IP database for the automatic mode to work properly. Go to <a href="%s">World Flags Options</a> and manually fetch the database.' ), admin_url( 'options-general.php?page=world_flags_extra' ) ) . '</td>';
}
function world_flags_admin_notice() {
	$db_exists = file_exists( ABSPATH . FLAGS_WEBDIR . "/data/IpToCountry.csv.gz" ) && file_exists( ABSPATH . FLAGS_WEBDIR . "/data/IpToCountry.csv" ) ;
 	if( !$db_exists && function_exists( "admin_url" ) )
		echo '<div class="error"><p>' . sprintf( __( 'You must download the IP database for the automatic mode to work properly. Go to <a href="%s">World Flags Options</a> and manually fetch the database.' ), admin_url( 'options-general.php?page=world_flags_extra' ) ) . '</p></div>';
}
